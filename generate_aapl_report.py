...
def make_report(df, metrics, out_pdf='AAPL_report_improved.pdf'):
    ...
    desc = "The shaded band projects a 30-day price range using the latest close and the historical daily log-return volatility. The daily σ is scaled by √30 to form σ₃₀ (the 30-day volatility), and the range uses last_price × exp(σ₃₀) for the upper bound and last_price × exp(-σ₃₀) for the lower bound, so it represents a probabilistic 1σ band rather than a point forecast."
